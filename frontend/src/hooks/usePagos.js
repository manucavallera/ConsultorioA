...
export const usePagos = () => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      // 🔍 DEBUG - Ver qué formato devuelve el backend
      console.log("Respuesta pagos:", pagosData);
      console.log("Respuesta estadísticas:", estadisticasData);
      console.log("Tipo de pagosData:", typeof pagosData);
      console.log("Es array pagosData:", Array.isArray(pagosData));

      // ✅ PROTECCIÓN PARA ASEGURAR QUE SEAN ARRAYS
      setPagos(pagosData.data || pagosData || []);
      setEstadisticas(estadisticasData.data || estadisticasData || {});
...
Para consultas, respondé este mensaje.`;
  ...
  const obtenerPagosPorPaciente = async (
    ...
    try {
      ...
      return pagosData.data || pagosData || [];
    } catch (err) {
