  ...
  try {
    ...
    if (pago.estado !== "pendiente") {
      return res.status(400).json({
        mensaje: "Solo se pueden editar pagos pendientes",
      });
    }

    // Actualizar campos
    if (monto) pago.monto = monto;
    if (fechaVencimiento) pago.fechaVencimiento = new Date(fechaVencimiento);
    ...
    res.status(500).json({
      mensaje: "Error al editar pago",
      error: error.message,
    });
  }
};

// 🗑️ ELIMINAR PAGO INDIVIDUAL
export const eliminarPago = async (req, res) => {
  try {
    ...
    const pagoIndex = cita.pago.pagos.findIndex(
      (p) => p.numeroPago === parseInt(numeroPago)
    );
    if (pagoIndex === -1) {
      return res.status(404).json({ mensaje: "Pago no encontrado" });
    ...
    const pago = cita.pago.pagos[pagoIndex];

    // No permitir eliminar pagos ya pagados
    if (pago.estado === "pagado") {
      return res.status(400).json({
        mensaje: "No se pueden eliminar pagos ya realizados",
      });
    }

    // Eliminar el pago del array
    cita.pago.pagos.splice(pagoIndex, 1);

    // Si no quedan pagos, eliminar el plan completo
    if (cita.pago.pagos.length === 0) {
      cita.pago = undefined;
    }
...
export const enviarRecordatorioEmail = async (req, res) => {
  try {
    const { citaId, numeroPago } = req.params;
    ...
    const asunto = `Recordatorio de Pago - Cita Médica`;
    const mensaje = `
      Estimado/a ${cita.pacienteId.nombre} ${cita.pacienteId.apellido},
      
      Le recordamos que tiene un pago pendiente:
      
      • Monto: $${pago.monto}
      • Cita: ${cita.fecha.toLocaleDateString()} a las ${cita.hora}
      • Vencimiento: ${pago.fechaVencimiento.toLocaleDateString()}
      
      Por favor, realice su pago a la brevedad para mantener su cita confirmada.
      
      Gracias por confiar en nosotros.
    `;

    // SIMULACIÓN - Aquí integrarías con servicio de email real
    console.log(`EMAIL ENVIADO a ${cita.pacienteId.email}:`);
    ...
    res.json({
      mensaje: "Recordatorio enviado por email",
      email: cita.pacienteId.email,
      asunto: asunto,
    });
  } catch (error) {
    res.status(500).json({
      mensaje: "Error al enviar recordatorio por email",
      error: error.message,
    });
  }
};
