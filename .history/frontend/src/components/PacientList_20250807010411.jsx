import { useEffect, useState, useMemo, useRef, useCallback } from "react"; // ← AGREGADO useRef y useCallback
...
export default function PacienteList({
  ...
  }, [API_URL, setPacientes]);

  useEffect(() => {
    // Solo hacer fetch si no se ha hecho antes Y no hay pacientes
    if (!hasFetched.current && pacientes.length === 0) {
      console.log("🔄 Primera carga de pacientes");
      fetchPacientes();
      hasFetched.current = true;
    } else {
      console.log("⏭️ Skip fetch - ya hay datos");
    }
  }, []); // ← Dependencias vacías

  // ✅ FIX: Usar useMemo en lugar de useEffect para evitar re-renders
  ...
  }, [pacientes, searchTerm]); // ← Mismo cálculo, pero con useMemo
