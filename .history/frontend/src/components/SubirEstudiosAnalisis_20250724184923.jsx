...
const SubirEstudioAnalisis = (props) => {
  ...
  // ✅ FUNCIÓN SEPARADA PARA CARGAR ESTUDIOS - URL CORREGIDA
  const cargarEstudios = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/estudios/solicitud/${solicitudId}`
      );
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ CORREGIDO: Agregado /api/ en la URL
      const res = await fetch("http://localhost:5000/api/estudios", {
        method: "POST",
        body: formData,
      });
