...
const PagosSimple = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      console.log("Respuestas:", { pagosRes, pacientesRes, citasRes });

      // Verificar si las respuestas son OK
      if (!pagosRes.ok) {
        console.error("Error en pagos:", pagosRes.status, pagosRes.statusText);
        setPagos([]);
      } else {
      ...
      setLoading(false);
      // Establecer arrays vacíos para que no crashee
      setPagos([]);
      setPacientes([]);
      setCitas([]);
    }
