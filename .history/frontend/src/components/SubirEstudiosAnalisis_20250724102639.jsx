...
// ✅ FUNCIÓN AGREGADA PARA ARREGLAR EL ERROR DE REACT
const renderAnalisis = (analisis) => {
  ...
  // Si es un objeto, extraer sus propiedades
  if (typeof analisis === "object" && analisis !== null) {
    ...
    // Si tiene otras propiedades, convertir a string
...
// ✅ FUNCIÓN PARA MOSTRAR SOLICITUDES EN EL SELECTOR
const getSolicitudDisplay = (solicitud) => {
  let analisisText = "";

  if (Array.isArray(solicitud.analisis)) {
    analisisText = solicitud.analisis.map((a) => renderAnalisis(a)).join(", ");
  } else if (solicitud.analisis) {
    analisisText = renderAnalisis(solicitud.analisis);
  } else {
    analisisText = "Sin análisis especificado";
  }
  ...
  return `${fecha} - ${analisisText}${descripcion}`;
};
...
const SubirEstudioAnalisis = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/solicitudes/paciente/${pacienteId}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Solicitudes recibidas:", data); // Para debug
        setSolicitudes(data);
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/solicitudes/${solicitudId}`)
      ...
      .then((data) => {
        console.log("Detalle solicitud recibido:", data); // Para debug
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/estudios/solicitud/${solicitudId}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Estudios recibidos:", data); // Para debug
        setEstudios(data);
      })
      .catch((error) => {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-100 p-6'>
      <div className='max-w-6xl mx-auto'>
        ...
        {/* ✅ DETALLE DE SOLICITUD - ARREGLADO */}
        {detalleSolicitud ? (
          <div className='bg-white rounded-2xl shadow-lg p-6 mb-8'>
            ...
            {/* ✅ ANÁLISIS SOLICITADOS - ARREGLADO PARA EVITAR ERROR DE REACT */}
            <div className='mt-6'>
        ...
        {/* ✅ SELECTOR DE SOLICITUD - ARREGLADO */}
        {!solicitudIdProp && (
          <div className='bg-white rounded-2xl shadow-lg p-6 mb-8'>
            <h3 className='text-xl font-bold text-gray-800 mb-4 flex items-center'>
