...
const PagosDashboard = () => {
  ...
  const guardarPago = async () => {
    ...
    try {
      ...
      const url = editandoPago
        ? `${API_URL}/pagos/${editandoPago._id}`
        : `${API_URL}/pagos`;

      const method = editandoPago ? "PUT" : "POST";
      ...
      if (response.ok) {
        ...
        alert(error.mensaje || "Error al guardar el pago");
      }
    } catch (error) {
  ...
  const editarPago = (pago) => {
    ...
    setSelectedPaciente(pago.pacienteId._id);
    setTipoTratamiento(pago.tipoTratamiento);
  ...
  const eliminarPago = async (pagoId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        alert(error.mensaje || "Error al eliminar el pago");
      }
    } catch (error) {
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-lg p-6 w-full max-w-md mx-4'>
            <h3 className='text-lg font-semibold mb-4'>
              {editandoPago ? "Editar Pago" : "Nuevo Pago"}
            </h3>
