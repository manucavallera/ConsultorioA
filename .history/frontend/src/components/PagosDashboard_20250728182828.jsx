...
const PagosSimple = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Probar cada ruta por separado (agregando puerto del backend)
      console.log("📡 Cargando pagos...");
      const pagosRes = await fetch("http://localhost:3000/pagos");
      console.log("Respuesta pagos:", pagosRes.status, pagosRes.statusText);

      console.log("📡 Cargando pacientes...");
      const pacientesRes = await fetch("http://localhost:3000/pacientes");
      console.log(
        "Respuesta pacientes:",
        pacientesRes.status,
        pacientesRes.statusText
      );

      console.log("📡 Cargando citas...");
      const citasRes = await fetch("http://localhost:3000/citas");
      console.log("Respuesta citas:", citasRes.status, citasRes.statusText);

      // Verificar si las respuestas son OK
      if (!pagosRes.ok) {
        console.error("❌ Error en pagos:", pagosRes.status);
        const errorText = await pagosRes.text();
        console.error("Contenido del error:", errorText.substring(0, 100));
        setPagos([]);
      } else {
        const contentType = pagosRes.headers.get("content-type");
        console.log("Content-Type pagos:", contentType);
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          console.error("❌ Respuesta de pagos no es JSON");
          setPagos([]);
        }
      }

      if (!pacientesRes.ok) {
        console.error("❌ Error en pacientes:", pacientesRes.status);
        setPacientes([]);
      } else {
        const contentType = pacientesRes.headers.get("content-type");
        if (contentType && contentType.includes("application/json")) {
          const pacientesData = await pacientesRes.json();
          console.log("✅ Pacientes cargados:", pacientesData);
          setPacientes(pacientesData.data || []);
        } else {
          console.error("❌ Respuesta de pacientes no es JSON");
          setPacientes([]);
        }
      }

      if (!citasRes.ok) {
        console.error("❌ Error en citas:", citasRes.status);
        setCitas([]);
      } else {
        const contentType = citasRes.headers.get("content-type");
        if (contentType && contentType.includes("application/json")) {
          const citasData = await citasRes.json();
          console.log("✅ Citas cargadas:", citasData);
          setCitas(citasData.data || []);
        } else {
          console.error("❌ Respuesta de citas no es JSON");
          setCitas([]);
        }
      }
  ...
  const cambiarEstado = async (pagoId, nuevoEstado) => {
    try {
      const response = await fetch(
        `http://localhost:3000/pagos/${pagoId}/confirmar`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const anexarCita = async (pagoId, citaId) => {
    try {
      const response = await fetch(
        `http://localhost:3000/pagos/${pagoId}/anexar-cita`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const enviarAlerta = async (pagoId) => {
    try {
      await fetch(`http://localhost:3000/pagos/${pagoId}/alerta`, {
        method: "POST",
      });
            ...
            <>
              ...
              {pago.metodoPago === "mercadopago" && (
                <button
                  onClick={() =>
                    window.open(
                      `https://mercadopago.com/checkout/${pago._id}`,
                      "_blank"
                    )
                  }
