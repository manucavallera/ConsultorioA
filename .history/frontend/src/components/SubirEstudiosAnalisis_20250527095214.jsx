...
const SubirEstudioAnalisis = ({ pacienteId }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/solicitudes/paciente/${pacienteId}`)
      .then((res) => {
        setSolicitudes(res.data);
        // Si hay solicitudes, setear la primera por defecto
        if (res.data.length > 0) setSolicitudId(res.data[0]._id);
      })
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/estudios/solicitud/${solicitudId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("/api/estudios", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      setArchivo(null);
      // Refrescar lista de estudios
      const res = await axios.get(`/api/estudios/solicitud/${solicitudId}`);
      setEstudios(res.data);
    } catch (err) {
      setErrorSubir(
        err?.response?.data?.message || "Error al subir el estudio"
      );
    }
    setSubiendo(false);
  };
