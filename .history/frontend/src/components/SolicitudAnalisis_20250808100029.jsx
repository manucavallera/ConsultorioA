...
export default function SolicitudAnalisis() {
 ...
 const handleGuardarValores = async () => {
  ...
  try {
    ...
    console.log('📊 Análisis procesados:', analisisConValores);

    // ✅ CAMBIO: Usar PUT en lugar de PATCH /valores
    const res = await fetch(`${API_URL}/${editandoValores}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        analisis: analisisConValores,
        // Mantener la descripción existente
        descripcion: solicitudes.find(s => s._id === editandoValores)?.descripcion
      }),
    });
