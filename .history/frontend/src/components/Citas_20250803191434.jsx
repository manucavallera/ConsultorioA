...
export default function CitasDashboard() {
  ...
  const enviarRecordatorioDoctor = (cita) => {
    const paciente = cita.pacienteId;
    const telefonoDoctor = numeroDoctor; // Tu número configurado
    const fechaFormateada = formatearFecha(cita.fecha);

    // 🔥 MENSAJE MEJORADO Y MÁS COMPLETO
    const mensaje = `🩺 *RECORDATORIO DE CITA MÉDICA* 📅

👤 *PACIENTE*
• Nombre: ${paciente.nombre} ${paciente.apellido}
• DNI: ${paciente.dni}
• Teléfono: ${paciente.telefono || "Sin teléfono"}
${paciente.email ? `• Email: ${paciente.email}` : ""}

📅 *DETALLES DE LA CITA*
• Fecha: ${fechaFormateada}
• Hora: ${formatearHora(cita.hora)}
• Tipo: ${cita.tipoConsulta} ${obtenerIconoTipo(cita.tipoConsulta)}
...
${paciente.obraSocial ? `🏥 *Obra Social:* ${paciente.obraSocial}` : ""}

✅ *Cita confirmada y programada*
⏰ *Estado: ${cita.estado}*

---
💡 *Tip: Este mensaje fue generado automáticamente por tu sistema de gestión de citas*`;

    // Codificar mensaje para URL (WhatsApp necesita esto)
    const mensajeCodificado = encodeURIComponent(mensaje);

    // URL de WhatsApp que abre con el mensaje ya escrito
    const urlWhatsApp = `https://wa.me/${telefonoDoctor}?text=${mensajeCodificado}`;

    try {
      // Abrir WhatsApp Web/App con el mensaje pre-escrito
      window.open(urlWhatsApp, "_blank", "width=600,height=700");

      // Feedback visual al usuario
      setMensaje(
        `📱 WhatsApp abierto con recordatorio de ${paciente.nombre} - Solo presiona ENVIAR`
      );
      setTimeout(() => setMensaje(""), 5000);

      // 🎯 OPCIONAL: Mostrar instrucciones
      if (
        confirm(
          "📱 WhatsApp se abrirá con el mensaje ya escrito.\n\n✅ Solo necesitas presionar ENVIAR\n\n¿Quieres continuar?"
        )
      ) {
        // El usuario confirmó, ya se abrió WhatsApp arriba
      }
    } catch (error) {
      console.error("Error al abrir WhatsApp:", error);
      setError("Error al abrir WhatsApp Web");
    }
  };
  const crearMensajeDetalladoDoctor = (cita) => {
    const paciente = cita.pacienteId;
    const fechaFormateada = formatearFecha(cita.fecha);

    let mensaje = `🩺 *RECORDATORIO DE CITA* 📅\n\n`;

    // Información del paciente
    mensaje += `👤 *PACIENTE*\n`;
    mensaje += `• Nombre: ${paciente.nombre} ${paciente.apellido}\n`;
    mensaje += `• DNI: ${paciente.dni}\n`;
    mensaje += `• Edad: ${paciente.edad || "No especificada"} años\n`;
    mensaje += `• Género: ${paciente.genero || "No especificado"}\n\n`;

    // Información de la cita
    mensaje += `📅 *DETALLES DE LA CITA*\n`;
    mensaje += `• Fecha: ${fechaFormateada}\n`;
    mensaje += `• Hora: ${formatearHora(cita.hora)}\n`;
    mensaje += `• Tipo: ${cita.tipoConsulta}\n`;
    mensaje += `• Duración: ${cita.duracion || 60} minutos\n`;
    ...
    mensaje += `\n📞 *CONTACTO*\n`;
    mensaje += `• Teléfono: ${paciente.telefono || "Sin teléfono"}\n`;
    if (paciente.email) {
    ...
    if (paciente.obraSocial) {
      mensaje += `🏥 *Obra Social:* ${paciente.obraSocial}\n`;
    }

    mensaje += `\n✅ *Cita confirmada y lista*`;
  ...
  const imprimirRecibo = (cita, pago, formato = "imprimir") => {
    if (formato === "pdf") {
      ...
      // Imprimir tradicional (código existente)
  ...
  const handleEditar = (cita) => {
    setFormData({
      ...
      fecha: cita.fecha.includes("T") ? cita.fecha.split("T")[0] : cita.fecha, // ✅ Fix zona horaria
      hora: cita.hora,
  ...
  const handleCambiarEstado = async (citaId, nuevoEstado) => {
    try {
      ...
      // 🆕 ENVIAR RECORDATORIO AL DOCTOR CUANDO SE PROGRAMA UNA CITA
      if (nuevoEstado === "Programada") {
        const cita = citas.find((c) => c._id === citaId);
        if (cita) {
          enviarRecordatorioDoctor(cita);
        }
      }
    } catch (err) {
      ...
      font-medium text-sm transition-all duration-200 flex items-center space-x-2'
                ...
                <div className='space-y-6'>
                  {citasFiltradas.map((cita) => (
                    <CitaCard
                      ...
                      onEnviarRecordatorio={enviarRecordatorioDoctor}
                    />
                  ))}
                </div>
              </div>
        ...
        {modalPagosAbierto && citaSeleccionadaPagos && (
          <ModalGestionPagos
            ...
            onConfirmarPago={confirmarPago}
            onImprimirRecibo={imprimirRecibo} // ✅ AGREGAR ESTA LÍNEA
...
const CitaCard = ({
  ...
  return (
    <div className='bg-white rounded-2xl border border-gray-200 hover:border-gray-300 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden'>
      ...
      font-semibold text-sm transition-all duration-200 flex items-center space-x-2 shadow-sm hover:shadow-md'
              title='Enviar recordatorio al doctor'
            >
              <span>📱</span>
...
const CalendarioCitas = ({ citas, onEditarCita }) => {
  ...
  const citasDelDia = (fecha) => {
    return citas.filter((cita) => {
      // ✅ Comparar solo fechas sin problemas de zona horaria
                  ...
                  >
                    ...
                    {editandoPago === pago.numeroPago ? (
                      ...
                      <div>
                        ...
                        {/* 🆕 SECCIÓN MODIFICADA - BOTONES PARA TODOS LOS PAGOS */}
                        <div className='flex flex-wrap gap-3'>
                          {/* Botones solo para pagos pendientes */}
                          {pago.estado === "pendiente" && (
                            <>
                              <button
                                onClick={() =>
                          ...
                          {/* 🆕 BOTONES PARA TODOS LOS PAGOS (PENDIENTES Y PAGADOS) */}
                          <button
                          ...
                          {/* 🆕 BOTONES DE IMPRESIÓN PARA TODOS LOS PAGOS */}
                          <div className='flex gap-2'>
                            <button
                              onClick={() =>
