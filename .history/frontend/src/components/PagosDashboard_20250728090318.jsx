...
import EstadisticasPagos from "./components/EstadisticasPagos";
...
export const usePagos = () => {
  ...
  const crearPago = async (datos) => {
    try {
      ...
      await cargarDatos();
      setMensaje("Pago creado exitosamente");
  ...
  const enviarWhatsApp = (pago) => {
    const paciente = pago.citaId.pacienteId;
    const mensaje = `Hola ${paciente.nombre}! 

💰 *Recordatorio de Pago*
📅 Cita: ${new Date(pago.citaId.fecha).toLocaleDateString("es-AR")}
🩺 Consulta: ${pago.citaId.tipoConsulta}
💵 Monto: $${pago.monto}
📋 Estado: ${pago.estado}
...
¡Gracias! 🙏`;
    ...
    const url = `https://wa.me/${paciente.telefono}?text=${encodeURIComponent(
      mensaje
    )}`;
