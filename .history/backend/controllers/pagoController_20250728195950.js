...
// Agrega estas dos funciones al final de tu pagoController.js
...
export const actualizarPago = async (req, res) => {
  try {
    ...
    const pago = await Pago.findByIdAndUpdate(id, datos, {
      new: true,
      runValidators: true,
    }).populate("pacienteId", "nombre apellido dni telefono");
...
export const eliminarPago = async (req, res) => {
  try {
    ...
    // Verificar si el pago tiene citas anexadas
    const citasAnexadas = await Cita.find({ pagoId: id });
    if (citasAnexadas.length > 0) {
      return res.status(400).json({
        mensaje: "No se puede eliminar un pago que tiene citas anexadas",
      });
    }
