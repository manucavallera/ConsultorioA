    ...
    pagos: [
      {
        ...
        // 🆕 ALERTAS EXPANDIDAS (WhatsApp + Email)
        alertaEnviada: { type: Boolean, default: false },
        fechaUltimaAlerta: Date,
        alertaWhatsAppEnviada: { type: Boolean, default: false },
        alertaEmailEnviada: { type: Boolean, default: false },
...
// 🆕 NUEVO MÉTODO: Verificar si necesita alerta específica
citaSchema.methods.necesitaAlertaTipo = function (tipo) {
  return (
    this.pago?.pagos?.some((pago) => {
      if (pago.estado !== "pendiente") return false;

      const campoAlerta =
        tipo === "whatsapp" ? "alertaWhatsAppEnviada" : "alertaEmailEnviada";
      if (pago[campoAlerta]) return false;
